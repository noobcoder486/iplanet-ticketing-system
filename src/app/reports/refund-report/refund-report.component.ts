...
export class RefundReportComponent implements OnInit {
  ...
  LocationCodeDD: DropDownValue = this.getBlankObject();
  LocationCode: string = '';
  RefundCode:String='';
  CompanyCode:string='';
  ...
  onLocation($event: { term: string; item: any[] }) {
    this.dropdownDataService.fetchDropDownData(DropDownType.Location, $event.term, {
    }).subscribe({
      next: (value) => {
        if (value != null) {
          this.LocationCodeDD = value;
        }
      },
      error: (err) => {
        this.LocationCodeDD = DropDownValue.getBlankObject();
      }
    });
  }
  ...
  exportReportData() {
    ...
    if ((this.StartDate != null || this.StartDate != undefined) && (this.EndDate != null || this.EndDate != undefined)) {
    {
      ...
      requestData.push({
        "Key": "RefundCode",
        "Value": this.RefundCode == null || this.RefundCode == undefined ? '' : this.RefundCode
      })
      requestData.push({
        "Key": "StartDate",
      ...
      this.reportService.downloadServiceReport('UNIVERSAL', contentRequest).subscribe(
        {
          next: (Value) => {
            try {
              ...
              link.href = url;
              const fileName = `Refund_Report_${startformattedDate}_to_${endformattedDate}.xls`;
            ...
            next: (Value) => {
              ...
              try {
                ...
                if (response.ReturnCode == '0') {
                  ...
                  this.results =[]
                  if (Array.isArray(data?.ReportData?.RefundReport)) {
                    this.results = data?.ReportData?.RefundReport
                  }
                  else {
                    this.results.push(data?.ReportData?.RefundReport)
                  }
