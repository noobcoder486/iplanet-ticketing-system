...
@Component({
  selector: 'app-table-replacement-detail-report',
  templateUrl: './table-replacement-detail-report.component.html',
  styleUrls: ['./table-replacement-detail-report.component.sass']
})
export class TableReplacementDetailReportComponent implements OnInit {
    ...
    results = [];
      ...
      columns: Columns[] = [
        ...
        { datatype: "STRING", field: "JobStatus", title: "JobStatus" },
        { datatype: "STRING", field: "JobStatusDescription", title: "JobStatusDescription" },
        ...
        { datatype: "STRING", field: "Type", title: "Type" },
    ...
    ngOnInit(): void {
      ...
      this.GetTableReplacementDetail()
  
    }
  
  
  
    ExportTableReplacementDetail() {
      ...
      requestData.push({
        "Key": "APIType",
        "Value": "ExportTableReplacementDetailReport"
      })
      requestData.push({
        "Key": "LocationCode",
      ...
      requestData.push({
        "Key": "SerialNo",
        "Value": this.SerialNo == null || this.SerialNo == undefined ? '' : this.SerialNo.trim()
      })
      requestData.push({
        "Key": "CompanyCode",
      ...
      this.reportService.downloadServiceReport('UNIVERSAL', contentRequest).subscribe(
        {
          next: (Value) => {
            try {
              ...
              link.href = url;
              const fileName = `TableReplacementDetail_Report_${startformattedDate}_to_${endformattedDate}.xls`;
    ...
    GetTableReplacementDetail() {
      ...
      requestData.push({
        "Key": "APIType",
        "Value": "GetTableReplacementDetailReport"
      })
      requestData.push({
        "Key": "LocationCode",
        "Value": this.LocationCode == null || this.LocationCode == undefined ? '' : this.LocationCode
          ...
          next: (Value) => {
            ...
            try {
              ...
              if (response.ReturnCode == '0') {
                ...
                this.results = Array.isArray(data?.TableReplacementDetailList?.TableReplacementDetail)
                  ? data?.TableReplacementDetailList?.TableReplacementDetail
                  : [data?.TableReplacementDetailList?.TableReplacementDetail];
                this.detail.next({ totalRecord: data?.TotalRecords, Data: this.results });
          ...
          this.dynamicService.getDynamicDetaildata(contentRequest).subscribe(
            {
              next: (Value) => {
                ...
                this.ngxSpinnerService.hide()
                    ...
                    // this.detail.next({ totalRecord: data?.TotalRecords, Data: data?.TableReplacementDetailList?.TableReplacementDetail });
                  }
                } catch (ext) {
                  console.log(ext);
                this.ngxSpinnerService.hide()
