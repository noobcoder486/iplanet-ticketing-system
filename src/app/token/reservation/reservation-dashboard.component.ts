...
export class ReservationDashboardComponent implements OnInit {
  ...
  // ReservationRequestDD= ['SENT FOR APPROVAL', 'PARTIALLY APPROVED', 'APPROVED', 'REJECTED']
  // ReservationDropDD= ['PENDING', 'CLOSED']
  // ReservationTypes = ['Request', 'Drop']
  ...
  // Dashboard List Part:- 
  // statuses: { ReservationType: string; ReservationDocType: string; JobStatusDesc: string; JobCount: number }[] = [];
  statusList: any[] = []
  pageTitle: String = "Pickup/Drop List";
  ...
  jobHeaderData: any[] = [];
  filtersEventPickup: BehaviorSubject<any> = new BehaviorSubject<any>(null);
  filtersEventDrop: BehaviorSubject<any> = new BehaviorSubject<any>(null);
  statuses: { ReservationType: string; JobStatusDesc: string; JobCount: number }[] = [];
  ReservationData: any[] = [];

  filterData: { ReservationStatusCode: string; ApprovalStatus: string; ReservationTypeCode: string } = {
    ReservationStatusCode:  '',
    ApprovalStatus: '',
    ReservationTypeCode: '',
  };
  
  toggleReservation: boolean = false;
  ...
  ngOnInit() {
    this.statuses = this.ReservationStatusDD.Data.map((index) => ({
      ReservationType: '',
   ...
   this.getStatusCount()
  //  this.statuses = this.ReservationRequestDD.map((status) => ({
  //     ReservationType: '',
  //     ReservationDocType: '',
  ...
  onCardSelected(status) {
    ...
    this.filterData.ReservationTypeCode = status.ReservationTypeCode;
    this.filterData.ReservationStatusCode = status.ReservationStatusCode;
    
    status.ReservationTypeCode == this.filtersEventPickup.next(this.filterData)
  }
    ...
    this.dynamicService.getDynamicDetaildata(contentRequest).subscribe(
      {
        next: (value )=>{
          ...
          if(response.ReturnCode == '0'){
            ...
            if(Array.isArray(extraData.Dashboard)){
              this.statusList = extraData.Dashboard
            }
            else{
              this.statusList.push(extraData.Dashboard)
            }
            console.log("Dashboard statuses ", this.statusList)

            let ReservationType: Set<string> = new Set();
            this.statusList.forEach( item =>{
              ReservationType.add( item.ReservationTypeDesc)
            })
            console.log("ReservationType ", ReservationType)

            this.ReservationData = [];
            for(let job of ReservationType) {
              let jobList = [];
              for(let jobItem of this.statusList){
                if(jobItem.ReservationTypeDesc == job){
                  jobItem.JobStatusDesc= jobItem.ReservationStatusDesc
                  jobItem.JobCount = jobItem.ReservationStatusCount
            ...
            // ReservationType.forEach( type =>{
            //   let sameTypeList = this.statusList.filter( item => item.ReservationTypeDesc == type)
            //   this.jobHeaderData.push( {
            //     "header": type,
            //     "list": sameTypeList
            //   });
            // })
            console.log("jobHeaderData ", this.jobHeaderData)
