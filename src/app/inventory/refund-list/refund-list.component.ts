...
export class RefundListComponent implements OnInit {
  ...
  screenDetail: any;
  actionDetails: any[]=[
    {"code": "APPROVE","icon": "remove_red_eye","title": "Aproval"}
  ];
  screen:any;
  Location: any;
  ...
  // RefundRequestDD= ['SENT FOR APPROVAL', 'PARTIALLY APPROVED', 'APPROVED', 'REJECTED']
  // RefundCreatedDD= ['PENDING', 'CLOSED']
  // RefundTypes = ['Request', 'Created']
  ...
  // Dashboard List Part:- 
  statuses: { RefundType: string; RefundDocType: string; JobStatusDesc: string; JobCount: number }[] = [];
  pageTitle: String = "Refund ";
  ...
  jobHeaderData: any[] = [];
  filtersEventRequested: BehaviorSubject<any> = new BehaviorSubject<any>(null);
  filtersEventCreated: BehaviorSubject<any> = new BehaviorSubject<any>(null);

  // filterData: { RefundStatus: string; ApprovalStatus: string; RefundDocType: string; RefundStatusCode: string } = {
  //   RefundStatus:  '',
  //   ApprovalStatus: '',
  //   RefundDocType: '',
  //   RefundStatusCode: '',
  // };
  filterData: {  RefundStatusCode: string } = {
    RefundStatusCode: '',
  };
  toggleRefund: boolean = false;
  ...
  //  this.statuses = this.RefundRequestDD.map((status) => ({
  //     RefundType: '',
  //     RefundDocType: '',
  ...
  onCardSelected(status) {
    console.log("Card selected event ", status)
    this.tabs[0].active = false;
    this.tabs[1].active = true;
    if (status.RefundTypeCode == 'REQUEST') {
      this.toggleRefund = true   
      this.filterData.RefundStatusCode = status.RefundStatusCode;
    } 
    else{
      this.toggleRefund = false
      this.filterData.RefundStatusCode = status.RefundStatusCode;
    }
    // this.filterData.ApprovalStatus = status.JobStatusDesc;
    // this.filterData.RefundDocType = status.RefundDocType;
    status.RefundTypeCode == 'REQUEST' ?
     this.filtersEventRequested.next(this.filterData)
     : this.filtersEventCreated.next(this.filterData);
  }
  ...
  async getStatusCount(){
      ...
      // console.log("Before Count SP:-", requestData);
      try {
   ...
   let data = JSON.parse(response?.ExtraData);
          console.log("Data is ", data);
           ...
           let dashboardList= []
             dashboardList = Array.isArray(data?.Dashboard) 
                ? data?.Dashboard 
                : [data?.Dashboard];
                 this.jobHeaderData=[]
            let headers = new Set(dashboardList.map(item => item.RefundTypeDesc))
            for( let header of headers){
              this.jobHeaderData.push( {
                "header": header,
                "list": dashboardList.filter(item => item.RefundTypeDesc == header)
              });
            }
      ...
      //   // Set Statuses
      //   this.jobHeaderData.push( {
      //     "header": refundtype == 'Request' ? "Refund Requests" : "Refund Created"  ,
      //     // "header": refundtype.TEXT,
      //     "list": this.statuses
      //   });
      //   // Reset the status data
      //   this.statuses = dd.map((status) => ({
      //     RefundType: '',
      //     RefundDocType: '',
      //     JobStatusDesc: status,
      //     JobCount: 0
      ...
      // }
      // this.ngxservice.hide();
      console.log("Statuses ",this.jobHeaderData)
  }
    ...
    this.dropdownDataService.fetchDropDownData(DropDownType.RefundDocType, $event.term, {
      ...
      next: (value) => {
        if (value != null) {
          ...
          // this.getStatusCount()
          // console.log("Refund Data :- ", this.RefundAgainstDD);
        }
      },
      error: (err) => {
  ...
  // async getStatusCount(){
  //   for (const refundtype of this.RefundTypes ){
  //     this.showSpinner =  true
  //     let dd = refundtype == 'Request' ? this.RefundRequestDD : this.RefundCreatedDD
  //     this.statuses = dd.map((status) => ({
  //       RefundType: '',
  //       RefundDocType: '',
  //       JobStatusDesc: status,
  ...
  //         status.RefundType = refundtype
  //         status.RefundDocType = 'RREFUND'
  //         let requestData =[]
  //         requestData.push({
  //           "Key": "APIType",
  //           "Value": refundtype == 'Request' ? "GetRefundRequestList" : "GetRefundList"
  //         });
  //         requestData.push({
  //           "Key":"ApprovalStatus",
  //           "Value": status.JobStatusDesc
  //         })
  //         requestData.push({
  //           "Key":"RefundDocType",
  //           "Value": ''
  ...
  //         requestData.push({
  //           "Key":"PageSize",
  //           "Value": '9999999' //Page Size is a large number to get all the rows
  //         });
  //         let strRequestData = JSON.stringify(requestData);
  //         let contentRequest =
  //         {
  ...
  //             let data = JSON.parse(response?.ExtraData);
  //             console.log("Data is ", data);
  //             data?.Totalrecords > 0 ? status.JobCount = data?.Totalrecords : status.JobCount =0 
  //           }
  //         } catch (err) {
  //           this.showSpinner = false
  //           // this.ngxservice.hide();
  ...
  //     for (const refundtype of this.RefundTypes) {
  //       const dd = refundtype === 'Request' ? this.RefundRequestDD : this.RefundCreatedDD;
  //       const statuses = dd.map(status => ({
  //         RefundType: refundtype,
  //         RefundDocType: 'RREFUND',
  //         JobStatusDesc: status,
  //         JobCount: 0
  //       }));
  
  //       for (const status of statuses) {
  //         const apiType = refundtype === 'Request' ? "GetRefundRequestList" : "GetRefundList";
  //         const approvalStatus = status.JobStatusDesc;
  //         const contentRequest = {
  //           "content": JSON.stringify([
  //             requestData,
  //             { "Key": "APIType", "Value": apiType },
  //             { "Key": "ApprovalStatus", "Value": approvalStatus },
  //             { "Key": "LocationCode", "Value": "" },
  //             { "Key": "RetailCustomerCode", "Value": "" },
  //             { "Key": "PageNo", "Value": '1' },
  //             { "Key": "PageSize", "Value": '9999999' }
  //           ])
  //         };
  
  //         observables.push(
  //           this.dynamicService.getDynamicDetaildata(contentRequest).pipe(
  //             catchError(err => {
  //               console.log(err);
  ...
  //           console.log("Data for each Observable ", data)
  //           this.jobHeaderData[i].list.JobCount = data?.Totalrecords > 0 ? data.Totalrecords : 0;
  //         }
  //       }
  //     }, error => {
  //       console.log(error);
  ...
  // async getStatusCountRequest(){
  //   this.showSpinner = true
  //   for (const refundtype of this.RefundAgainstDD.Data ){
  //     // console.log("RefundDoc ", refundtype )
  //     if ( refundtype.Id == 'RREFUND'){
  //       for (const status of this.statuses) {
  //               status.RefundDocType = refundtype.Id
  //               let requestData =[]
  //               requestData.push({
  //                 "Key": "APIType",
  ...
  //       // Set Statuses
  //       this.jobHeaderData.push( {
  //         "header": "Repair (Advance Payment) Requests",
  //         // "header": refundtype.TEXT,
  //         "list": this.statuses
  //       });
  //       // Reset the status data
  ...
  //       // Set Statuses
  //       this.jobHeaderData.push( {
  //         "header": "Repair (Advance Payment) Created",
  //         // "header": refundtype.TEXT,
  //         "list": this.statuses
  //       });
  //       // Reset the status data
