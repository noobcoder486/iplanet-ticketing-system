...
@Component({
  selector: 'app-happy-calling',
  templateUrl: './happy-calling.component.html',
  styleUrls: ['./happy-calling.component.sass']
})
export class HappyCallingComponent implements OnInit {
        ...
        if (response.ReturnCode == '0') {
          ...
          if (data.Totalrecords != '0') {
           ...
           console.log('Leads Return Object', data);
            ...
            this.JobObject = data.LeadManagement.JobObject  
            // this.LeadDisposition = data?.LeadDisposition 
            // this.Remarks = data?.Remark
            this.LeadDisposition = this.LeadObject?.LeadDisposition 
            this.Remarks = this.LeadObject?.Remark
  ...
  returnPrevious() {
    this.route.navigateByUrl('/auth/' + glob.getCompanyCode() + '/happy-calling-list');
  }
    ...
    this.dynamicService.getDynamicDetaildata(contentRequest).subscribe({
      next: (value) => {
        ...
        if (response.ReturnCode == '0') {
          ...
          this.route.navigate(['/auth/'+glob.getCompanyCode()+'/happy-calling-list'])
  ...
  onLeadDisposition($event: { term: string; item: any[] }) {
    this.dropdownDataService.fetchDropDownData(DropDownType.HappyCallingDisposition, $event.term, {
