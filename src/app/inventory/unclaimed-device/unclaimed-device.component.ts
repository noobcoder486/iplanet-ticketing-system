...
@Component({
  selector: 'app-unclaimed-device',
  templateUrl: './unclaimed-device.component.html',
  styleUrls: ['./unclaimed-device.component.css']
})
export class UnclaimedDeviceComponent implements OnInit {
  constructor(
      ...
      this.dynamicService.getDynamicDetaildata(contentRequest).subscribe(
        {
          next: (value) => {
            ...
            if (response.ReturnCode == '0') {
              this.toastMessage.success("Successfully Parts Added")
              this.route.navigate(["auth/" + glob.getCompanyCode() + "/unclaimed-device"], { queryParams: { headerguid: this.deliveryChallanGUID } })
              this.ngxSpinnerService.hide();
            }
            else {
              this.errorMessage = response.ReturnMessage;
  ...
  GetUnclaimedDevicesList() {
    let requestData = [];
    requestData.push({
      "Key": "ApiType",
      "Value": "GetUnclaimedDeviceList"
    });
    requestData.push({
      "Key": "CompanyCode",
      "Value": glob.getCompanyCode()
        ...
        next: (value) => {
          ...
          if (response.ReturnCode == '0') { 
            ...
            if ( data.Totalrecords > 0 ){
                Array.isArray(data.UnclaimedDeviceList.UnclaimedDevice) ?
                  this.popUpArray = data.UnclaimedDeviceList.UnclaimedDevice :
                      this.popUpArray = [data.UnclaimedDeviceList.UnclaimedDevice]
            }
            else{
