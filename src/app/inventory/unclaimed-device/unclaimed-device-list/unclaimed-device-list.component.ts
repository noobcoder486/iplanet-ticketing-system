...
@Component({
  selector: 'app-unclaimed-device-list',
  templateUrl: './unclaimed-device-list.component.html',
  styleUrls: ['./unclaimed-device-list.component.css']
})
export class UnclaimedDeviceListComponent implements OnInit {
  ...
  // Dashboard List Part:- 
  DeliveryChallanData: any[] = [];
  statuses: { DeliveryChallanType: string; JobStatusDesc: string; JobCount: number }[] = [];
  pageTitle: String = "Unclaimed Devices";
  ...
  filterDataRequest: { DeliveryChallanType: string, DeliveryChallanStatusCode: string; DeliveryChallanStatusDesc: string; LocationCode: string; SelectedDocType :string } = {
    DeliveryChallanType: '',
  ...
  actionEmit(event){
    ...
    if(event.action == 'EDIT'){
      this.route.navigate(['/auth/'+glob.getCompanyCode()+'/unclaimed-device'], { queryParams: { headerguid: event.row.DeliveryChallanGUID} })
    }
  }

  ngOnInit(): void {
    // Dashboard Part:-
    this.statuses = this.DeliveryChallanTypeDD.Data.map((index) => ({
      DeliveryChallanType: '',
  ...
  ReturnToLocation(){
    this.route.navigate(['/auth/'+glob.getCompanyCode()+'/return-unclaimed-device/']);
  }
  ...
  onCardSelected(status) {
    ...
    this.filterDataRequest.SelectedDocType = status.ReturnTypeCode;
  ...
  getStatusCount(){
    this.showSpinner()
          ...
          next: (Value) => {
            ...
            try {
              ...
              if(response.ReturnCode =='0'){
                ...
                ReturnTypeDesc=[...set];
                this.DeliveryChallanData = [];
                for(let job of ReturnTypeDesc) {
                  let jobList = [];
                  for(let jobItem of response.ExtraDataJSON.Dashboard){
                    if(jobItem.ReturnTypeDesc == job){
                      jobItem.JobStatusDesc= jobItem.DeliveryChallanStatusDesc
                      jobItem.JobCount = jobItem.DeliveryChallanStatusCount
                      jobList.push(jobItem);
                    }
                  }
                  this.DeliveryChallanData.push( {
                    "header": job,
                    "list": jobList
                  });
                }
              }
              console.log("Job List ", this.DeliveryChallanData)
            } catch (ext) {
              this.ngxservice.hide()
              console.log(ext);
            }
      ...
      //   "list": this.statuses
      // });
      // this.statuses = this.DeliveryChallanTypeDD.Data.map((status) => ({
      //   SalesReturnType: '',
  ...
  add(){
    this.route.navigate(['/auth/'+glob.getCompanyCode()+'/unclaimed-device/']);
  }
