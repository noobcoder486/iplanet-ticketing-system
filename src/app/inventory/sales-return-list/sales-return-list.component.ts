...
export class SalesReturnListComponent implements OnInit {
  ...
  // Dashboard List Part:- 
  statuses: { SalesReturnType: string; JobStatusDesc: string; JobCount: number }[] = [];
  ...
  filterDataRequest: { ApprovalStatus: string; LocationCode: string } = {
    ApprovalStatus: '',
    LocationCode: '',
  };
  filterDataCreated: { SalesReturnStatus: string; LocationCode: string } = {
    SalesReturnStatus: '',
    LocationCode: '',
  };

  selectedGrid: 'REQUEST' | 'CREATED' = 'REQUEST'; 
  Spinner = false

  ngOnInit(): void {
    
    // Dashboard Part:-
    this.statuses = this.ApprovalStatusDD.map((status) => ({
      SalesReturnType: '',
  ...
  onCardSelected(status) {
    ...
    this.selectedGrid = status.SalesReturnTypeCode;
    this.tabs[0].active = false;
    this.tabs[1].active = true;
  
    if (status.SalesReturnTypeCode === 'REQUEST') {
      this.filterDataRequest.ApprovalStatus = status.SalesReturnStatusCode
      // this.filterDataRequest.LocationCode = this.LocationCode;
      this.filtersEventRequest.next(this.filterDataRequest);
    } 
    else {
      this.filterDataCreated.SalesReturnStatus = status.SalesReturnStatusCode;
      // this.filterDataCreated.LocationCode = this.LocationCode;
      this.filtersEventCreated.next(this.filterDataCreated);
    }
  }
  ...
  getRequestStatusCount(){
    ...
    this.statuses.forEach( ( status) =>{
      status.SalesReturnType = 'Request'
      let requestData =[]
      requestData.push({
        "Key": "APIType",
    ...
    requestData.push({
      ...
      this.statuses = this.SalesReturnStatusDD.map((status) => ({
        SalesReturnType: '',
        JobStatusDesc: status,
        JobCount: 0
      }));
  }
  getCreatedStatusCount(){
    this.statuses.forEach( (status) =>{
      status.SalesReturnType = 'Created'
      let requestData =[]
      requestData.push({
        "Key": "APIType",
  ...
  // Search(statusname){
  //   this.GetSalesReturnRequestList(statusname)
  // }
  // GetSalesReturnRequestList(statusname) {
  //   let requestData =[]
  //   requestData.push({
  //     "Key": "APIType",
  //     "Value": "GetSalesReturnRequestList"
  ...
  redirectToSalesReturnPage(item)
  {
    ...
    item == '' ? 
      this.route.navigate(['/auth/' +glob.getCompanyCode() + '/sales-return-request/'])
        : this.route.navigate(['/auth/' +glob.getCompanyCode() + '/sales-return/'], {queryParams: { salesreturnguid:item.SalesReturnGuid  ,locationcode:item.LocationCode  ,customercode:item.RetailCustomerCode}})
  }
