...
@Component({
  selector: 'app-dropoff-management-list',
  templateUrl: './dropoff-management-list.component.html',
  styleUrls: ['./dropoff-management-list.component.css']
})
export class DropoffManagementListComponent implements OnInit {
  ...
  // Dashboard List Part:- 
  DropoffData: any[] = [];
  statuses: { DropoffType: string; JobStatusDesc: string; JobCount: number }[] = [];
  ...
  filterDataRequest: {  DropoffStatusCode: string } = {
  ...
  onCardSelected(status) {
    ...
    this.filterDataRequest.DropoffStatusCode = status.DropoffStatusCode
      ...
      this.dynamicService.getDynamicDetaildata(contentRequest).subscribe(
        {
          next: (Value) => {
            ...
            try {
                ...
                for(let job of ReturnTypeDesc) {
                  ...
                  for(let jobItem of response.ExtraDataJSON.Dashboard){
                    if(jobItem.ReturnTypeDesc == job){
                      jobItem.JobStatusDesc= jobItem.DropoffStatusDesc
                      jobItem.JobCount = jobItem.DropoffStatusCount
              ...
              console.log("Job List ", this.DropoffData)
            } catch (ext) {
              this.ngxservice.hide()
              console.log(ext);
            }
  ...
  add(){
    this.route.navigate(['/auth/'+glob.getCompanyCode()+'/dropoff-management-parts/']);
  }
