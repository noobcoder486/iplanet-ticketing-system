...
@Component({
  selector: 'app-apple-registry-list',
  templateUrl: './apple-registry-list.component.html',
  styleUrls: ['./apple-registry-list.component.css']
})
export class AppleRegistryListComponent implements OnInit {
  ...
  pageTitle: String = "Apple Registry";
  ...
  actionEmit(event){
    ...
    if(event.action == 'EDIT'){
      this.route.navigate(['/auth/'+glob.getCompanyCode()+'/apple-registry'], { queryParams: { headerguid: event.row.AppleRegistryGUID} })
    }
  }
  ...
  add(){
    this.route.navigate(['/auth/'+glob.getCompanyCode()+'/apple-registry/']);
  }
