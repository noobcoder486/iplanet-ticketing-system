...
@Component({
  selector: 'app-bulk-return-order-dc-list',
  templateUrl: './bulk-return-order-dc-list.component.html',
  styleUrls: ['./bulk-return-order-dc-list.component.css']
})
export class BulkReturnOrderDcListComponent implements OnInit {
   ...
   pageTitle: String = " Bulk Return Order DC";
   ...
   actionEmit(event){
     ...
     if(event.action == 'EDIT'){
       this.route.navigate(['/auth/'+glob.getCompanyCode()+'/create-bulk-return-dc'], { queryParams: { BulkReturnOrderDCHeaderGUID: event.row.BulkReturnOrderDCHeaderGUID} })
     }
   }
   ...
   getStatusCount(){
     this.showSpinner()
       ...
       requestData.push({
         "Key": "APIType",
         "Value": "GetBulkReturnOrderDCDashboard"
       });
       requestData.push({
         "Key": "CompanyCode",
   ...
   add(){
     this.route.navigate(['/auth/'+glob.getCompanyCode()+'/create-bulk-return-dc/']);
   }
