...
export class NretManagementComponent implements OnInit {
    ...
    this.dynamicService.getDynamicDetaildata(contentRequest).subscribe(
      {
        next: (value) => {
          ...
          if (response.ReturnCode == '0') {
            ...
            this.NretType=data?.DeliveryChallanSubType ?? ''
            this.getToLocationData()
  ...
  showAddParts(){
    ...
    if (this.DeliveryChallanType == null || this.DeliveryChallanType == undefined) {
     ...
     else if(this.NretType == null || this.NretType == undefined ||  this.NretType  == ''){
      this.toastMessage.error('Please Select Nret Type...');
      return
    }
      ...
      this.dropdownDataService.fetchDropDownData(DropDownType.NRETTYPES, $event.term, {
        ...
        next: (value) => {
          if (value != null) {
            ...
            console.log("onNretTypes  nret comp", this.NretTypeDD)
          }
        },
        error: (err) => {
