...
@Component({
  selector: 'app-onsite-process',
  templateUrl: './onsite-process.component.html',
  styleUrls: ['./onsite-process.component.css']
})
export class OnsiteProcessComponent implements OnInit {
  ...
  actionDetails: any[]=[
    {"code": "VIEW","icon": "remove_red_eye","title": "Create Job"},
  ];
  columns: Columns[] = [
  ...
  actionEmit(event){
    ...
    if(event.action == 'VIEW'){ 
         this.route.navigate(['/auth/' +glob.getCompanyCode() + '/reservation/'], {queryParams: { rc:event.row.ReservationCode }})
    }
  ...
  getRepairDetails() {
    ...
    this.gsxService.getRepairDetails(LocationCode, CompanyCode, contentRequest).subscribe(
    {
      next: (value) => {
        ...
        console.log("OnsiteList ", response);
        ...
        next: (Value) => {
          ...
          try {
            ...
            if (response.ReturnCode == '0') {
              ...
              if( Array.isArray(data.OnsiteList?.Onsite) ) {
                this.OnsiteList = data.OnsiteList?.Onsite;
              }
              else{
                this.OnsiteList.push(data.OnsiteList?.Onsite)
              }
